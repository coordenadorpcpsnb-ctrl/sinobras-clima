...
# ══════════════════════════════════════════════════════════════════════════
# 3. SELECIONAR MELHOR MODELO: SARIMAX vs XGBoost
#    Holdout: últimos 36 meses (3 anos) para comparação
#    Critério: RMSE no holdout — menor ganha
# ══════════════════════════════════════════════════════════════════════════
print("\n[3/6] Comparando SARIMAX(N34+TSA+PDO) vs XGBoost…")

import xgboost as xgb
from sklearn.metrics import mean_squared_error

# Série completa
endog_all = pd.Series(d['prec'].values, index=pd.PeriodIndex(d['date2'], freq='M'))
exog_all  = pd.DataFrame({'n34_l23': d['n34_l23'].values,
                           'tsa_l23': d['tsa_l23'].values,
                           'pdo_l23': d['pdo_l23'].values}, index=endog_all.index)

HOLDOUT = 36   # 3 anos de holdout

# ── Splits ──────────────────────────────────────────────────────────────
endog_tr  = endog_all.iloc[:-HOLDOUT]
endog_te  = endog_all.iloc[-HOLDOUT:]
exog_tr   = exog_all.iloc[:-HOLDOUT]
exog_te   = exog_all.iloc[-HOLDOUT:]
y_true    = endog_te.values

# ── Modelo A: SARIMAX ────────────────────────────────────────────────────
print("  Treinando SARIMAX…")
res_sar = SARIMAX(endog_tr, exog=exog_tr, order=(1,0,2), seasonal_order=(1,1,1,12),
                  enforce_stationarity=False).fit(disp=False)
fc_sar  = res_sar.get_forecast(steps=HOLDOUT, exog=exog_te)
y_sar   = np.clip(fc_sar.predicted_mean.values, 0, None)
rmse_sar = float(np.sqrt(mean_squared_error(y_true, y_sar)))
mae_sar  = float(np.mean(np.abs(y_true - y_sar)))
ss_res   = np.sum((y_true - y_sar)**2)
ss_tot   = np.sum((y_true - y_true.mean())**2)
r2_sar   = float(1 - ss_res/ss_tot) if ss_tot > 0 else 0.0
print(f"  SARIMAX — RMSE={rmse_sar:.1f}mm  MAE={mae_sar:.1f}mm  R²={r2_sar:.4f}")

# ── Modelo B: XGBoost ────────────────────────────────────────────────────
print("  Treinando XGBoost…")

def build_xgb_features(df):
    """Features: índices ENSO + mês (sazonalidade) + lags de prec."""
    X = pd.DataFrame({
        'mes':       df.index.month if hasattr(df.index, 'month') else df['mes'],
        'mes_sin':   np.sin(2*np.pi*df.index.month/12),
        'mes_cos':   np.cos(2*np.pi*df.index.month/12),
        'n34_l23':   df['n34_l23'].values,
        'tsa_l23':   df['tsa_l23'].values,
        'pdo_l23':   df['pdo_l23'].values,
    })
    return X

# Usar o DataFrame d que tem todos os índices
prec_all = d['prec'].values
n_tr = len(d) - HOLDOUT

# Features treino
X_tr = pd.DataFrame({
    'mes':      [p.month for p in endog_tr.index],
    'mes_sin':  np.sin(2*np.pi*np.array([p.month for p in endog_tr.index])/12),
    'mes_cos':  np.cos(2*np.pi*np.array([p.month for p in endog_tr.index])/12),
    'n34_l23':  exog_tr['n34_l23'].values,
    'tsa_l23':  exog_tr['tsa_l23'].values,
    'pdo_l23':  exog_tr['pdo_l23'].values,
    'prec_l1':  np.concatenate([[np.nan]*1,  prec_all[:n_tr-1]]),
    'prec_l2':  np.concatenate([[np.nan]*2,  prec_all[:n_tr-2]]),
    'prec_l12': np.concatenate([[np.nan]*12, prec_all[:n_tr-12]]),
})
X_tr = X_tr.dropna().reset_index(drop=True)
y_tr = prec_all[n_tr-len(X_tr):n_tr]

# Features teste (índices ENSO conhecidos)
d_te = d.iloc[-HOLDOUT:].copy()
# endog_te.index é PeriodIndex
...
# Previsão no holdout — iterativa (cada passo usa prec prevista anterior)
y_xgb   = []
prec_buf = list(prec_all[:-HOLDOUT])   # buffer de prec conhecida

for j_te, (_, row) in enumerate(d_te.iterrows()):
    p_te = endog_te.index[j_te]   # Period do mês de teste
    X_step = pd.DataFrame([{
        'mes':      p_te.month,
        'mes_sin':  np.sin(2*np.pi*p_te.month/12),
        'mes_cos':  np.cos(2*np.pi*p_te.month/12),
        'n34_l23':  row['n34_l23'],
        'tsa_l23':  row['tsa_l23'],
        'pdo_l23':  row['pdo_l23'],
    ...
    pred = float(max(0, model_xgb.predict(X_step)[0]))
    y_xgb.append(pred)
    prec_buf.append(pred)

y_xgb    = np.array(y_xgb)
rmse_xgb = float(np.sqrt(mean_squared_error(y_true, y_xgb)))
mae_xgb  = float(np.mean(np.abs(y_true - y_xgb)))
ss_res_x = np.sum((y_true - y_xgb)**2)
r2_xgb   = float(1 - ss_res_x/ss_tot) if ss_tot > 0 else 0.0
print(f"  XGBoost  — RMSE={rmse_xgb:.1f}mm  MAE={mae_xgb:.1f}mm  R²={r2_xgb:.4f}")

# ── Selecionar o melhor modelo ───────────────────────────────────────────
MODELO_SELECIONADO = 'SARIMAX' if rmse_sar <= rmse_xgb else 'XGBoost'
print(f"\n  🏆 Modelo selecionado: {MODELO_SELECIONADO} "
      f"(RMSE {min(rmse_sar,rmse_xgb):.1f}mm vs {max(rmse_sar,rmse_xgb):.1f}mm)")

# Ranking para o bundle
MODEL_RANKING = [
    {'modelo':'SARIMAX(N34+TSA+PDO)', 'rmse':round(rmse_sar,1),
     'mae':round(mae_sar,1), 'r2':round(r2_sar,4), 'selecionado': MODELO_SELECIONADO=='SARIMAX'},
    {'modelo':'XGBoost(ENSO+Saz+Lags)', 'rmse':round(rmse_xgb,1),
     'mae':round(mae_xgb,1), 'r2':round(r2_xgb,4), 'selecionado': MODELO_SELECIONADO=='XGBoost'},
]
MODEL_RANKING.sort(key=lambda x: x['rmse'])

# ── Treinar o modelo vencedor na série COMPLETA ──────────────────────────
print(f"  Retreinando {MODELO_SELECIONADO} na série completa ({len(d)} meses)…")

if MODELO_SELECIONADO == 'SARIMAX':
    endog = endog_all
    exog  = exog_all
    ...
    endog = endog_all
    exog  = exog_all
    # Retreinar XGBoost na série completa
    prec_all_full = d['prec'].values
    X_full = pd.DataFrame({
        'mes':      [p.month for p in endog_all.index],
        'mes_sin':  np.sin(2*np.pi*np.array([p.month for p in endog_all.index])/12),
        'mes_cos':  np.cos(2*np.pi*np.array([p.month for p in endog_all.index])/12),
        'n34_l23':  exog_all['n34_l23'].values,
        'tsa_l23':  exog_all['tsa_l23'].values,
        'pdo_l23':  exog_all['pdo_l23'].values,
        'prec_l1':  np.concatenate([[np.nan]*1,  prec_all_full[:-1]]),
        'prec_l2':  np.concatenate([[np.nan]*2,  prec_all_full[:-2]]),
        'prec_l12': np.concatenate([[np.nan]*12, prec_all_full[:-12]]),
    })
    X_full = X_full.dropna().reset_index(drop=True)
    y_full = prec_all_full[len(prec_all_full)-len(X_full):]
    model_xgb.fit(X_full, y_full)
    prec_buf_full = list(prec_all_full)
    b1 = b2 = b3 = 0.0
    print(f"  XGBoost retreinado: {len(X_full)} amostras")
...
# Horizonte: mês atual → mês atual + 11 meses
H_start   = pd.Period(f'{TODAY.year}-{TODAY.month:02d}', 'M')
ano_base  = TODAY.year   # ano base para TRI e build_n34_path
...
skip      = H_start.ordinal - first_fc.ordinal       # passos a pular até mês atual
if skip < 0:
    skip = 0   # série já alcançou o mês atual — usar imediatamente
...
for name, A in SCEN.items():
    ...
    if MODELO_SELECIONADO == 'SARIMAX':
        # Previsão pontual + intervalo de confiança via simulação Monte Carlo
        ...
        prec_fc = mean[skip:skip+12]
        lo95_fc = ci.iloc[skip:skip+12, 0].values
        hi95_fc = ci.iloc[skip:skip+12, 1].values
        p5_fc   = np.percentile(sims[skip:skip+12,:], 5,  axis=1)
        p95_fc  = np.percentile(sims[skip:skip+12,:], 95, axis=1)

    else:  # XGBoost — previsão iterativa por cenário ENSO
        prec_buf_sc = list(d['prec'].values)   # buffer de prec conhecida
        preds_sc    = []
        for j, p in enumerate(H_full):
            X_step = pd.DataFrame([{
                'mes':      p.month,
                ...
                'tsa_l23':  tsa_fc[j],
                'pdo_l23':  pdo_fc[j],
                'prec_l1':  prec_buf_sc[-1]  if len(prec_buf_sc) >= 1  else 0,
                'prec_l2':  prec_buf_sc[-2]  if len(prec_buf_sc) >= 2  else 0,
                'prec_l12': prec_buf_sc[-12] if len(prec_buf_sc) >= 12 else 0,
            }])
            pred = float(max(0, model_xgb.predict(X_step)[0]))
            preds_sc.append(pred)
            prec_buf_sc.append(pred)

        preds_arr = np.array(preds_sc)
        prec_fc   = preds_arr[skip:skip+12]
        # IC 95% via bootstrap perturbação nos features
        boot_preds = []
        np.random.seed(42 + hash(name) % 1000)
        for _ in range(300):
            noise = np.random.normal(0, rmse_xgb*0.15, 12)
            boot_preds.append(np.clip(prec_fc + noise, 0, None))
        boot_arr = np.array(boot_preds)
        lo95_fc  = np.percentile(boot_arr, 2.5,  axis=0)
        hi95_fc  = np.percentile(boot_arr, 97.5, axis=0)
        p5_fc    = np.percentile(boot_arr, 5,    axis=0)
        p95_fc   = np.percentile(boot_arr, 95,   axis=0)

    results[name] = {
        'labels': [f"{p.month:02d}/{str(p.year)[2:]}" for p in H_want],
        'prec':   prec_fc.round(1).tolist(),
        'lo95':   lo95_fc.round(1).tolist(),
        'hi95':   hi95_fc.round(1).tolist(),
        'p5':     p5_fc.round(1).tolist(),
        'p95':    p95_fc.round(1).tolist(),
        'n34':    [path.get((p.year,p.month), 0.0) for p in H_want],
    }

print(f"  ✅ {len(results)} cenários gerados ({MODELO_SELECIONADO})")
...
rjm = rc   # resultado já em jul→jun (aba Balanço Hídrico usa jul→jun)

_LABELS_JUL_JUN = ['Jul','Ago','Set','Out','Nov','Dez','Jan','Fev','Mar','Abr','Mai','Jun']
bh_final = {
    'cenarios':    bh_results,
    'clim':        {'p':[round(v,1) for v in cm[6:]+cm[:6]],  # jul→jun
                    'etp':ETP_JUL_JUN,
                    'etr':[r['etr'] for r in rjm],'def':[r['def'] for r in rjm],
                    'exc':[r['exc'] for r in rjm],'arm':[r['arm'] for r in rjm],
                    'labels':_LABELS_JUL_JUN,
...
# Climatologia reordenada para o mês atual (índice 0 = mês atual)
# cm é jan-dez (índice 0=jan); offset do mês atual em relação a jan:
_m0         = TODAY.month - 1          # 0-based: ago=7
clim_atual  = cm[_m0:] + cm[:_m0]     # roda para começar no mês atual
clim_jun_mai = [round(v,1) for v in clim_atual]
...
# Plano Operacional — logica de campo jul→jun (fixo)
html = html.replace(
    "{m:'Jun', t:'o'}, {m:'Jul', t:'o'}, {m:'Ago', t:'o'}, {m:'Set', t:'v'},\n    {m:'Out', t:'d'}, {m:'Nov', t:'d'}, {m:'Dez', t:'c'}, {m:'Jan', t:'c'},\n    {m:'Fev', t:'c'}, {m:'Mar', t:'c'}, {m:'Abr', t:'d'}, {m:'Mai', t:'v'},",
    "{m:'Jul', t:'o'}, {m:'Ago', t:'o'}, {m:'Set', t:'v'}, {m:'Out', t:'d'},\n    {m:'Nov', t:'d'}, {m:'Dez', t:'c'}, {m:'Jan', t:'c'}, {m:'Fev', t:'c'},\n    {m:'Mar', t:'c'}, {m:'Abr', t:'d'}, {m:'Mai', t:'v'}, {m:'Jun', t:'o'},"
)

# nino_seasons — iniciar no mês atual (igual ao fc_labels)
_meses_pt = ['jan','fev','mar','abr','mai','jun','jul','ago','set','out','nov','dez']
_ns = []
_y, _m = TODAY.year, TODAY.month
for _ in range(12):
    _ns.append(f"'{_meses_pt[_m-1]}/{str(_y)[2:]}'")
    _m += 1
    if _m > 12: _m, _y = 1, _y + 1
html = re.sub(r'nino_seasons:\[[^\]]+\]', f'nino_seasons:[{",".join(_ns)}]', html)

# BH_LABELS_HYD — igual ao D.fc_labels (mês atual)
html = re.sub(
    r"BH_LABELS_HYD\s*=\s*[^;\n]+",
    "BH_LABELS_HYD = D.fc_labels   // mesmo eixo X da aba Previsão",
    html
)

# BH_CLIM_HYD estático — arrays pré-rotados para o mês atual (sem JS dinâmico)
_off_bh   = (TODAY.month - 7) % 12
_clim_rot = {f: bh_final['clim'][f][_off_bh:] + bh_final['clim'][f][:_off_bh]
             for f in ['p','etp','etr','def','exc','arm']}
_etp_rot  = ETP_JAN_DEZ[TODAY.month-1:] + ETP_JAN_DEZ[:TODAY.month-1]

_new_clim_hyd = (
    f'/* BH_CLIM_HYD pré-rotado para {TODAY.strftime("%m/%Y")} '
    f'(gerado em {TODAY.strftime("%d/%m/%Y")}) */\n'
    'BH_CLIM_HYD = {\n'
    + '\n'.join(f'  {f}: {json.dumps([round(v,2) for v in _clim_rot[f]])},'
                 for f in ['p','etp','etr','def','exc','arm'])
    + '\n};'
)
html = re.sub(
    r'/\* BH_CLIM_HYD.*?BH_CLIM_HYD = \{.*?\};',
    _new_clim_hyd, html, flags=re.DOTALL
)

# ETP pré-rotada em buildBhEtrComp
_etp_str = json.dumps(_etp_rot)
html = re.sub(
    r'// ETP pré-rotada.*?\n  const etp = \[[^\]]+\];',
    f'// ETP pré-rotada para {TODAY.strftime("%m/%Y")}\n  const etp = {_etp_str};',
    html
)
...
# Atualizar BH.clim — arrays e labels (jul→jun)
bc = bh_final['clim']
for field in ['p','etp','etr','def','exc','arm']:
    bh_block = re.sub(
        rf'(clim:.*?{field}:\s*)(\[[^\]]+\])',
        lambda mo, nv=json.dumps(bc[field]): mo.group(1)+nv,
        bh_block, flags=re.DOTALL, count=1)
bh_block = re.sub(
    r"(clim:.*?labels:\s*)(\[[^\]]+\])",
    lambda mo: mo.group(1)+"['Jul','Ago','Set','Out','Nov','Dez','Jan','Fev','Mar','Abr','Mai','Jun']",
    bh_block, flags=re.DOTALL, count=1)
...
new_bundle = json.dumps({
    ...
    'meta_best':   {'modelo':MODELO_SELECIONADO,
                    'rmse_holdout':round(min(rmse_sar,rmse_xgb),1),
                    'rmse_sar':round(rmse_sar,1), 'rmse_xgb':round(rmse_xgb,1),
                    'r2_sar':round(r2_sar,4), 'r2_xgb':round(r2_xgb,4),
